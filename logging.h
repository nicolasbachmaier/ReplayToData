...
{
	_globalCvarManager->log("[Replay-To-Date] " + std::vformat(format_str, std::make_format_args(std::forward<Args>(args)...)));
}

template <typename... Args>
void LOG(std::wstring_view format_str, Args&&... args)
{
	_globalCvarManager->log("[Replay-To-Date] " + std::vformat(format_str, std::make_wformat_args(std::forward<Args>(args)...)));
}
